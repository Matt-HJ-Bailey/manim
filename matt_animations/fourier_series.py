...
class FourierOfBreadCat(FourierOfPiSymbol):
    CONFIG = {
        "n_vectors": 75,
        "run_time": 20,
        "start_drawn": False,
    ...
    def get_drawn_path(self, *args, **kwargs):
        ...
        for index in range(PATH_SIZE):
            this_color = cmap(index / PATH_SIZE)[:3]
            this_color = matplotlib.colors.rgb2hex(this_color)
            print(f"prism({index / PATH_SIZE}={this_color}")
            path[index].set_color(this_color)
        return path

class FourierOfBarford(FourierOfPiSymbol):
    CONFIG = {
        "n_vectors": 101,
        "run_time": 20,
        "start_drawn": False,
        "file_name": "bad_boy_barford",
        "height": 10,
    }
    ...
    def get_shape(self):
        shape = SVGMobject("bad_boy_barford")
        shape.center()
        self.add(shape)
        return shape
